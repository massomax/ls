...
export default function MockScreen() {
  ...
  const items = useMemo(() => {
    ...
    if (sort === "rank")
      arr.sort((a, b) => b.favoritesCount - a.favoritesCount);
    return arr;
  }, [query, sort, hot]);
  ...
  return (
    <div className="min-h-screen bg-lp-bg">
      <TopBar
        query={query}
        onQuery={setQuery}
        subtitle="Preview: tokens + components"
      />
        ...
        <div className="mt-5 grid grid-cols-1 gap-4 sm:grid-cols-2 lg:grid-cols-4">
          {items.map((p) => (
            <ProductCard
              ...
              onGo={() =>
                alert(
                  "Переход во внешний магазин (в будущем: click + externalUrl)",
                )
              }
            />
          ))}
        ...
        {active ? (
          <div className="space-y-3">
            <div className="rounded-2xl border border-lp-border bg-white p-4">
              ...
              <div className="mt-2 text-2xl font-extrabold text-lp-text">
                {active.price} ₽
              </div>
            ...
            <div className="text-sm text-lp-muted">
              Здесь в будущем можно подставить реальные поля из backend
              (offerType, метрики, ссылка). В UI kit только каркас.
            </div>
          </div>
        ) : null}
      </Sheet>
