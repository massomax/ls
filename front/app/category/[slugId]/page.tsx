...
function CategoryPageContent({ slugId }: { slugId: string }) {
  const router = useRouter();
  const { query, setQuery } = useSearch();
  const { slug, id } = useMemo(() => parseSlugId(slugId), [slugId]);

  const categoriesQ = useCategories();
  const subcategoriesQ = useSubcategories(id);
  ...
  const category = useMemo(
    () => (categoriesQ.data ?? []).find((item) => item.id === id) ?? null,
    [categoriesQ.data, id],
  );
  ...
  return (
    <div className="space-y-4">
      <div className="flex items-center gap-2 text-sm text-lp-muted">
        ...
        <span>/</span>
        <span className="text-lp-text">{category?.name ?? slug}</span>
      </div>

      <SectionTitle title={category?.name ?? "Категория"} hint="Категории" />
                        ...
                        subcategoriesQ.data.map((subcategory) => {
                          ...
                          return (
                            <button
                              ...
                              className="rounded-lg px-3 py-1.5 text-left text-sm text-lp-text hover:bg-slate-50"
                            >
                              {subcategory.name}
                            </button>
                          );
...
export default function CategoryPage({
  ...
  return (
    <Suspense fallback={<PageStateSkeleton rows={8} />}>
      <CategoryPageContent slugId={slugId} />
    </Suspense>
  );
}
